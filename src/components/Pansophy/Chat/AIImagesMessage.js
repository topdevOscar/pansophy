...
const AiImagesMessage = ({ data,updateImageAPIData, ...props }) => {
  ...
  // const data = useSelector((state) => state.chats.data.images);
  console.log(data)
 ...
 const handleDownloadAll = () => {
  ...
  return (
    <div className="flex md:space-x-4 w-full">
      ...
      {setting?.dall_logo ?
          <img src={setting?.dall_logo} alt="Dall-e-2 Icon" /> : <svg
          width="16"
          height="17"
          viewBox="0 0 16 17"
          fill="none"
      ...
      <div className="flex flex-nowrap items-start justify-between space-x-10 p-4 w-full bg-pansophy-bgDark rounded-xl self-center leading-7 text-sm text-white">
        <div className="flex flex-col items-start space-y-4 w-full">
            ...
            {data &&
              data.map((image, imageIdx) => (
                ...
                >
                  ...
                  <div
                    className={`flex items-center justify-center space-x-2 absolute top-0 left-0 w-full h-full bg-[#126949]/90 rounded group-hover:visible ${selected.includes(image.media_id) ? "visible" : "invisible"
                      }`}
                  >
                    <button
                      type="button"
                    ...
                    >
                      <svg
                        ...
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className={`${selected.includes(image.media_id)
                          ? "fill-[#10B981]"
                          : "fill-white"
                          }`}
                      >
          ...
          <Tooltip id="Selected  Images Copied " />
            <button
            //  data-tooltip-id="Selected  Images Copied "
            //  data-tooltip-content={__("Selected  Images Copied ", "pansophy")}
            onClick={()=>{
              copyImagesAll()
            ...
            <Tooltip id="All Images Copied " />
            <button
            //  data-tooltip-id="All Images Copied "
            //  data-tooltip-content={__("All Images Copied ", "pansophy")}
              onClick={() => {
                copyImages()
